...
## Handle other plugins' updates (run_context `capability_changes`)
Each tool carries `capability_changes` with `added` / `updated` / `removed`. When `changed` is true, this plugin (or another) was installed/updated/removed.
- **Merge into routing, never overwrite local content.** Only edit inside the `<!-- ai-reflect:auto BEGIN/END -->` block; everything the user wrote outside it stays untouched.
- For `added`/`updated` capabilities: add or refresh a "when to invoke this" routing hint based on the new name/description, so the new ability actually gets used.
- For `removed`: drop its routing hint (this is editing our own sentinel block, allowed; it is not the pruning-gated deletion of a tool's config).
- Surface the changes to the user as a short list with Apply/Skip choices before syncing, not as a text prompt.
...
## 处理其他插件的更新（run_context 的 `capability_changes`）
每个工具带 `capability_changes`，含 `added` / `updated` / `removed`。`changed` 为真即说明本插件或别的插件被装/更新/移除了。
- **合并进路由，绝不覆盖本地内容。** 只动 `<!-- ai-reflect:auto BEGIN/END -->` 区块内的内容；用户写在区块外的一律不碰。
- 对 `added`/`updated` 的能力：根据新的 name/description 新增或刷新一条"何时调用它"的路由提示，让新能力真正被用上。
- 对 `removed`：去掉它的路由提示（这是改我们自己的哨兵区块，允许；不是受瘦身闸限制的"删工具配置"）。
- 同步前把变化做成简短清单 + 应用/跳过选择题给用户，不要文本框提问。
