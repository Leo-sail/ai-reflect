...
If the user did not specify the period/scope, **offer choices instead of a text box**: period (weekly / monthly / yearly / custom range) and depth (full / core only). Only "custom" leads to typing.
...
若用户没指定周期/范围，**给选择题而非文本框**：周期（周报 / 月报 / 年报 / 自定义范围）、详略（全套 / 仅核心）。只有"自定义"才让用户打字。
...
The range comes from `$ARGUMENTS` (default weekly). If the user specified custom content/format requirements, follow the user. Show the report as a draft first; after a yes/no confirm, write it to `~/.ai-reflect/local/reports/<date>-<period>.md` and return the highlights. The report is also redacted, containing no keys or verbatim private data.

范围取自参数 $ARGUMENTS（默认周报）。若用户指定了自定义内容/格式要求，以用户要求为准。先把报告当草稿展示，经一个是/否确认后再写到 `~/.ai-reflect/local/reports/<日期>-<周期>.md` 并把要点回给用户。报告内容同样过脱敏，不含密钥/逐字隐私。
